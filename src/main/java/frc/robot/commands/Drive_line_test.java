...
public class Drive_line_test extends CommandBase {
 ...
 // private DigitalInput m_input_DVal4 = new DigitalInput(12);
  ...
  private final OnBoardIO m_boardIO;
  ...
  public Drive_line_test(double speed_f, double speed_t, double speed_s, Drivetrain drive, OnBoardIO boardIO) {
...
stop_drive = false;
  ...
  @Override
  public void execute() {
    if (m_boardIO.getinputDVal0()){Sensor_val0 = 1;}
    if (m_boardIO.getinputDVal1()) {Sensor_val1 = 2;}
    if (m_boardIO.getinputDVal2()) {Sensor_val2 = 4;}
    if (m_boardIO.getinputDVal3()) {Sensor_val3 = 8;}
    if (m_boardIO.getinputDVal4()) {Sensor_val4 = 16;}

    Sensor_val = (Sensor_val0 + Sensor_val1 + Sensor_val2 + Sensor_val3 + Sensor_val4);
    
    SmartDashboard.putBoolean("DIO 0", m_boardIO.getinputDVal0());
    SmartDashboard.putBoolean("DIO 1", m_boardIO.getinputDVal1());
    SmartDashboard.putBoolean("DIO 2", m_boardIO.getinputDVal2());
    SmartDashboard.putBoolean("DIO 3", m_boardIO.getinputDVal3());
    SmartDashboard.putBoolean("DIO 4", m_boardIO.getinputDVal4());
...
/*  if (Sensor_val == 4){
  ...
  */                
  ...
  if (Sensor_val == 8 || Sensor_val == 16){
    left_speed = ( m_speed_f * -m_speed_t);
    right_speed = ( m_speed_f );
  }
  if (Sensor_val == 2 || Sensor_val == 1){
  left_speed = ( m_speed_f );
  right_speed = ( m_speed_f * -m_speed_t );
  }
  if (Sensor_val == 4){
...
//check to see if more than 1 sensor sees the line
if (m_boardIO.getinputDVal2() &&  (m_boardIO.getinputDVal4())){
  left_speed = 0;
  right_speed = 0;
  stop_drive = true;
}
